...
/**/
    ...
    var activation_form = jQuery("#digits_setting_update");

    activation_form.on("submit", function (e) {
        if(jQuery(this).hasClass("dig_activation_form_zhaket")) return; /**/
        e.preventDefault();
    ...
    jQuery(window).on('digits_admin_save', function (e) {
        ...
        if(jQuery("#dig_purchasecode").length > 0) return;
        if (save_timer) {
...
/**/
    ...
    function updateSettings(dac, activate) {
        is_submitted = false;
        //telemetry();/* برای حذف کدهای بررسی رفتار مشتری*/
        updateDashboardView();
    ...
    try {
        /* Added for some js error*/
        if (typeof field_data !== typeof undefined) dig_custom_field_data = jQuery.parseJSON(field_data);
    ...
    function show_create_new_field_panel() {
        // lockScroll();/*نصف صفحه فیلدها باز نمیشد غیرفعال کردم*/
    ...
    function hide_custom_panel() {
        ...
        //unlockScroll();/*نصف صفحه فیلدها باز نمیشد غیرفعال کردم*/
    }
    ...
    jQuery(document).on("click", ".dig_call_test_api_btn", function () {
        ...
        if (jQuery(".dig_admin_submit").attr("disabled")) {
            digCallTestApi();
        } else jQuery("#digits_setting_update").trigger("submit");
    ...
    jQuery(".imp_exp_btn_fn").on('click', function () {
        ...
        if (export_import_stage == 1) {
            ...
            try {
                ...
                jQuery("#digits_setting_update").trigger("submit");
            } catch (e) {
    ...
    jQuery("#digits-update").find('p').each(function () {
        jQuery(this).find('.thickbox').attr({
            'href': 'https://demo.mersa-co.ir/digits/changelog',
            'target': '_blank',
            'class': ''
        });
    });
...
/************ Customization ***************/
jQuery("#dig_reset_custom_fields").on('click', function () {
    var loader = jQuery(".dig_load_overlay");
    var txt;
    var r = confirm("آیا مطمئن هستید که می خواهید تمام فیلدهایی که خودتان اضافه کردید را حذف کنید؟");
    if (r == true) {
        loader.show();
        jQuery.ajax({
            type: 'post',
            ...
            url: digsetobj.ajax_url,
            data: {
                action: 'dig_reset_custom_fields'
            },
            success: function (res) {
                loader.hide();
                if(res == 1) showDigNoticeMessage('،فیلدهای اختصاصی این افزونه حذف شد. صفحه را رفرش نمایید'); else showDigErrorMessage('مشکلی در اجرای دستور شما اتفاق افتاد. در صورتی که این مشکل باقی بود با پشتیبان تماس بگیرید.');
            },
            error: function (res) {
                loader.hide();
                showDigErrorMessage('مشکلی در اجرای دستور شما اتفاق افتاد. در صورتی که این مشکل باقی بود با پشتیبان تماس بگیرید.');
            }
        });
    }
});
jQuery(".dig_activation_form_zhaket").on("submit", function(){
